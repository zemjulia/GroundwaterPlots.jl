...
db = PyCall.PyNULL()
interp = PyCall.PyNULL()
np = PyCall.PyNULL()
mc = PyCall.PyNULL()
PathEffects = PyCall.PyNULL()
function __init__()
	try
		copy!(db, PyCall.pyimport("aquiferdb"))
	catch
		@warn "GroundwaterDataBase is not available!"
	end

	try
		copy!(interp, PyCall.pyimport("scipy.interpolate"))
	catch
		@warn "SciPy is not available!"
	end

	try
		copy!(np, PyCall.pyimport("numpy"))
	catch
		@warn "NumPy is not available!"
	end

	try
		copy!(mc, PyCall.pyimport("matplotlib.colors"))
		copy!(PathEffects, PyCall.pyimport("matplotlib.patheffects"))
	catch
		@warn "MatPlotLib is not available!"
	end
end
...
function cmap_alpha(cmap::PyPlot.ColorMap, center::Number, width::Number; method::Symbol=:inv_gauss)
	...
	# Alpha will be 0 for x < b, 1 for x > c, and linearly increasing from b to c.
...
function addwells(ax, x::AbstractVector{Number}, y::AbstractVector{Number}; colorstring="k.", markersize=20, fontsize=14, alpha=1.0)
...
function addwells_beta(ax, wellnames::AbstractVector{String}; xoffset=5, yoffset=5, colorstring="k.", markersize=20, fontsize=14, alpha=1.0)
...
function addwells(ax, wellnames::AbstractVector; xoffset=15, yoffset=15, colorstring="k.", markersize=20, fontsize=14, alpha=1.0, smartoffset::Bool=true)
...
function addwells(ax, wellnames::AbstractVector{String}, wells::Dict{String,Any}; colorstring="k.", markersize=20, fontsize=14, alpha=1.0)
	for well in wellnames
...
"""
function addobservations(ax, observations; text::Bool=true, fontsize=12, markersize=40, color="red", offsets = Dict("P-1"=>[20,-25],"P-2"=>[20,-15],"P-3"=>[20,-10]))
	vec_length = length(keys(observations))
	xvec = Array{Float64,1}(undef, vec_length)
	yvec = Array{Float64,1}(undef, vec_length)
	tvec = Array{String,1}(undef, vec_length)
...
function getwelloffset(wellname; offset_dict=nothing, default=[15,15])
	...
	if offset_dict === nothing
		offset_dict = Dict("R-35a"=>set_southeast,"CrIN-5"=>set_west,
			"CrEX-3"=>set_southeast,"CrIN-2"=>set_south,"CrIN-4"=>set_southeast,
...
function interpolationplot(boundingbox, xs::AbstractVector, ys::AbstractVector, plotdata::AbstractVector; upperlimit::Bool=false, lowerlimit::Bool=false, cmap=rainbow, figax::Bool=false, alpha::Number=1.0)
	boundingbox = resizeboundingbox(boundingbox)
	x0, y0, x1, y1 = boundingbox
	numxgridpoints=1920
	numygridpoints=1080
	gridxs = [x for x in range(x0, x1; length=numxgridpoints), y in range(y0, y1; length=numygridpoints)]
	gridys = [y for x in range(x0, x1; length=numxgridpoints), y in range(y0, y1; length=numygridpoints)]
	if interp != PyCall.PyNULL()
		gridinerp = interp.griddata((xs, ys), plotdata, (gridxs, gridys), method="linear")
		return interpolationplot(boundingbox, gridinerp; upperlimit=upperlimit, lowerlimit=lowerlimit, cmap=cmap, figax=figax, alpha=alpha)
	else
		@warn "SciPy is not available!"
		return nothing
	end
...
function interpolationplot(boundingbox, xs::AbstractVector, ys::AbstractVector, plotdata::AbstractVector, cov; upperlimit::Bool=false, lowerlimit::Bool=false, cmap=rainbow, pretransform=x->x, posttransform=x->x, figax::Bool=false, alpha::Number=1.0)
	boundingbox = resizeboundingbox(boundingbox)
	x0, y0, x1, y1 = boundingbox
	numxgridpoints=1920
	numygridpoints=1080
	gridxs = [x for x in range(x0, x1; length=numxgridpoints), y in range(y0, y1; length=numygridpoints)]
	gridys = [y for x in range(x0, x1; length=numxgridpoints), y in range(y0, y1; length=numygridpoints)]
	gridzs = map(posttransform, Kriging.krige(permutedims(hcat(gridxs[:], gridys[:])), permutedims(hcat(xs, ys)), map(pretransform, plotdata), cov))
	griddata = reshape(gridzs, numxgridpoints, numygridpoints)
	return interpolationplot(boundingbox, griddata; upperlimit=upperlimit, lowerlimit=lowerlimit, cmap=cmap, figax=figax, alpha=alpha)
end
# Plot data using inverse weighted distance
function interpolationplot(boundingbox, xs::AbstractVector, ys::AbstractVector, plotdata::AbstractVector, pow::Number; upperlimit::Bool=false, lowerlimit::Bool=false, cmap=rainbow, pretransform=x->x, posttransform=x->x, figax::Bool=false, alpha::Number=1.0)
	boundingbox = resizeboundingbox(boundingbox)
	x0, y0, x1, y1 = boundingbox
	numxgridpoints=1920
	numygridpoints=1080
	gridxs = [x for x in range(x0, x1; length=numxgridpoints), y in range(y0, y1; length=numygridpoints)]
	gridys = [y for x in range(x0, x1; length=numxgridpoints), y in range(y0, y1; length=numygridpoints)]
	gridzs = map(posttransform, Kriging.inversedistance(permutedims(hcat(gridxs[:], gridys[:])), permutedims(hcat(xs, ys)), map(pretransform, plotdata), pow))
	griddata = reshape(gridzs, numxgridpoints, numygridpoints)
	return interpolationplot(boundingbox, griddata; upperlimit=upperlimit, lowerlimit=lowerlimit, cmap=cmap, figax=figax, alpha=alpha)
end
# Create an empty plot with the background image, but no data
function interpolationplot(boundingbox; alphabackground=1.0)
	boundingbox = resizeboundingbox(boundingbox)
	x0, y0, x1, y1 = boundingbox
...
# Plot matrix data
function interpolationplot(boundingbox, gridvariabale::AbstractMatrix; upperlimit::Bool=false, lowerlimit::Bool=false, cmap=rainbow, figax::Bool=false, alpha::Number=0.7, alphabackground::Number=1.0)
	if figax == false
		fig, ax = interpolationplot(boundingbox; alphabackground=1.0)
	else
		fig, ax = figax[1], figax[2]
	end
	boundingbox = resizeboundingbox(boundingbox)
	x0, y0, x1, y1 = boundingbox
	upperlimit = upperlimit == false ? maximum(gridvariabale) : upperlimit
	lowerlimit = lowerlimit == false ? minimum(gridvariabale) : lowerlimit
	gridvariabale_m = map(x->(x < lowerlimit) ? NaN : (x > upperlimit ? upperlimit : x), permutedims(gridvariabale))
	img = ax.imshow(gridvariabale_m)
	# img = ax.imshow(gridvariabale_m, origin="lower", extent=[x0, x1, y0, y1], cmap=cmap, interpolation="nearest", alpha=alpha, vmin=lowerlimit, vmax=upperlimit)
	return fig, ax, img
end
...
$(DocumentFunction.documentfunction(interpolationplot;
argtext=Dict("boundingbox"=>"bounding box",
			...
			"pow"=>"power parameter",
			"gridvariabale"=>"grid to create plot on"),
...
function getticks(plotdata::AbstractVector; nstep::Number=5, sigdigits::Integer=1)
	i = .!isnan.(plotdata)
...
function getticksold(lowerlimit::Number, upperlimit::Number; nstep::Number=5, sigdigits::Integer=1)
	ticks = map(x->round(x, sigdigits), range(lowerlimit, upperlimit; length=nstep))
	return ticks
end
function getticks(lowerlimit::Number, upperlimit::Number; nstep::Number=5, sigdigits::Integer=1, quiet::Bool=true)
	...
	dxr = round(dx; sigdigits=sigdig, base=rbase)
	mn = round(lowerlimit; sigdigits=sigdig, base=rbase)
	mx = round(upperlimit; sigdigits=sigdig, base=rbase)
